...
bool fun(bool a, int c)
{
    int lowCounter = -900; // minimum reading of shift plus play VINYL STOP effect
    bool ret = a;
    counter = c;
    if (a)
        for (counter; counter >= lowCounter; counter -= 1)
        {
            // counter = _pwm;

            if (!digitalRead(BTN1) && !digitalRead(BTN4))
            {
                vTaskDelay(100 / portTICK_PERIOD_MS);
                while (!digitalRead(BTN1) && !digitalRead(BTN4))
                    vTaskDelay(10 / portTICK_PERIOD_MS);
                // isPlaying = !isPlaying;
                ret = fun(!a, counter);
                break;
            }
            vTaskDelay(10 / portTICK_PERIOD_MS); // shift plus play vinyl stop effect - how fast it takes to stop, increase number to make it slower
        }
    else
        for (counter; counter <= rotCounter; counter += 1)
        {
            // counter = _pwm;
...
// bool fun(bool a, int c)
// {
//     const int lowCounter = -900;
//     int target = a ? lowCounter : rotCounter;
//     int step = a ? -1 : 1;
//     counter = c;

//     while ((a && counter >= target) || (!a && counter <= target))
//     {
//         counter += step;
...
{
    (void)pvParameters;
    double upLimit = 100, downLimit = -100; // shift plus increase tempo / shift plus decrease tempo
    ...
    while (1)
    {
        if (btn4State && btn1State)
        {
            ...
            bypass = true;
            // isPlaying=!isPlaying;
            fun(!isPlaying, !isPlaying ? rotCounter : -900);
            // if (isPlaying)
            // {
            //     fun(true, rotCounter);
            // counter = rotCounter;
...
void setup()
{
    ...
    else
    {
        ...
        xTaskCreatePinnedToCore(TaskMotor, "TaskPrint", 1024, NULL, 1, NULL, ARDUINO_RUNNING_CORE);
    }
}
void loop()
{
