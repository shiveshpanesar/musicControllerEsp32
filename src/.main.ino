...
#define MPIN 26
// #define RTOS
double motorMinSpeed = 42535; // change motor default voltage at 0
// double motorMinSpeed = 0; // change motor default voltage at 0
double motorMaxSpeed = 65535;
double counterMinSpeed = -100;
double counterMaxSpeed = 100;
int doubleTapTime = 200; // double Tap timing
...
void IRAM_ATTR updateEncoder()
{
    ...
    double boost = 0.1; // boost for step increase/decrease
    ...
    if (currentStateCLK != lastStateCLK)
    {
        ...
        {
            btn4State ? counter += boost : counter += step;
        }
        else
        {
            btn4State ? counter -= boost : counter -= step;
        }
        counter = constrain(counter, counterMinSpeed, counterMaxSpeed);
        rotCounter = counter;
    }
...
{
    (void)pvParameters;
    double upLimit = 50, downLimit = -50; // hardcoded values for
    double step = 10;                      // increase/decrease step
    ...
    while (1)
    {
        ...
        else if (isPlaying && btn3State) // if playing and buttton 2 is pressed
        {
            counter < (counterMaxSpeed - step) ? counter += step : counter = counterMaxSpeed;
            vTaskDelay(25 / portTICK_PERIOD_MS);
        }
        else if (isPlaying && btn2State) // if pllaying and btoon 3 is pressed]
        {
